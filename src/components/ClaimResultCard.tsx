...
export default function ClaimResultCard({ claim }: { claim: ClimateClaim }) {
  const technique = getTechniqueById(claim.technique);
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="rounded-2xl border border-accent/30 bg-accent/10 p-5">
        <p className="text-xs font-semibold uppercase tracking-wide text-accent">Risposta in 10 secondi</p>
        <p className="mt-2 text-lg font-medium">{claim.tenSecondReply}</p>
        <div className="mt-4">
          <CopyButton text={claim.tenSecondReply} label="Copia la risposta breve" />
        </div>
      </section>

      <section>
        <h2 className="text-sm font-semibold uppercase tracking-wide text-foreground/50">
          Perché questa frase convince
        </h2>
        <p className="mt-2 text-foreground/85">{claim.whyItPersuades}</p>
      </section>

      <section>
        <h2 className="text-sm font-semibold uppercase tracking-wide text-foreground/50">Reframe emotivo</h2>
      ...
      <section>
        <div className="flex items-center justify-between gap-4">
          ...
          <CopyButton text={claim.longerReply} label="Copia" />
        </div>
      ...
      <section className="rounded-2xl border border-accent-science/30 bg-accent-science/5 p-5">
        <h2 className="text-sm font-semibold uppercase tracking-wide text-accent-science">Backup scientifico</h2>
        <p className="mt-2 text-foreground/85">{claim.scientificBackup}</p>
        {technique && (
          <p className="mt-3 text-sm text-foreground/60">
            Tecnica manipolatoria: <span className="font-medium text-foreground/80">{technique.name}</span> —{" "}
      ...
      <section>
        <h2 className="text-sm font-semibold uppercase tracking-wide text-foreground/50">Angolo italiano ed europeo</h2>
        <p className="mt-2 text-foreground/85">{claim.italianAngle}</p>
      </section>

      <section>
        <h2 className="text-sm font-semibold uppercase tracking-wide text-foreground/50">Fonti</h2>
        <div className="mt-3">
      ...
      <section>
        <h2 className="text-sm font-semibold uppercase tracking-wide text-foreground/50">Card da condividere</h2>
        <div className="mt-3">
